...
const struct CKPhotoBrowserControllerNotification CKPhotoBrowserControllerNotification = {
	.didNavigateToPhoto = @"CKPhotoBrowserControllerNotification.didNavigateToPhoto",
	.willDismiss = @"CKPhotoBrowserControllerNotification.willDismiss",
	.didDismiss = @"CKPhotoBrowserControllerNotification.didDismiss",
};

static const CGFloat CKPhotoBrowserControllerOverlayAnimationDuration = 0.2;
static const CGFloat CKPhotoBrowserControllerInterPhotoSpacing = 16.0;
static const UIEdgeInsets CKPhotoBrowserControllerCloseButtonImageInsets = {3, 0, -3, 0};

@interface CKPhotoBrowserController () <UIPageViewControllerDataSource, UIPageViewControllerDelegate, CKPhotoBrowserChildControllerDelegate>

- (instancetype)initWithCoder:(NSCoder *)aDecoder NS_DESIGNATED_INITIALIZER;

@property (nonatomic) UIPageViewController *pageViewController;
@property (nonatomic) CKPhotoBrowserTransitionController *transitionController;
@property (nonatomic) UIPopoverController *activityPopoverController;

@property (nonatomic) UIPanGestureRecognizer *panGestureRecognizer;
@property (nonatomic) UITapGestureRecognizer *singleTapGestureRecognizer;

@property (nonatomic) CKPBControllerOverlayView *overlayView;

// 自定义notificationCenter，作用限于`CKPhotoBrowserController`的实例
@property (nonatomic) NSNotificationCenter *notificationCenter;

@property (nonatomic) BOOL shouldHandleLongPress;
@property (nonatomic) BOOL overlayWasHiddenBeforeTransition;

@property (nonatomic, readonly) CKPhotoBrowserChildController *currentPhotoViewController;
@property (nonatomic, readonly) UIView *referenceViewForCurrentPhoto;
@property (nonatomic, readonly) CGPoint boundsCenterPoint;

@property (nonatomic, nullable) id<CKPhotoBrowserItem> initialPhoto;
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    return [self initWithDataSource:[CKPBArrayDataSource dataSourceWithPhotos:@[]] initialPhoto:nil delegate:nil];
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    if (self) {
        [self commonInitWithDataSource:[CKPBArrayDataSource dataSourceWithPhotos:@[]] initialPhoto:nil delegate:nil];
    }
...
- (void)didReceiveMemoryWarning {
	...
	// 内存释放
	for (int index = 0; index < self.dataSource.numberOfPhotos.integerValue; index++) {
		[[self.dataSource photoAtIndex:index] unloadUnderlyingImage];
	}
}

#pragma mark - CKPhotoBrowserController
...
- (instancetype)initWithDataSource:(id <CKPhotoBrowserDataSource>)dataSource initialPhotoIndex:(NSInteger)initialPhotoIndex delegate:(nullable id <CKPhotoBrowserControllerDelegate>)delegate {
    id <CKPhotoBrowserItem> initialPhoto = [dataSource photoAtIndex:initialPhotoIndex];
    return [self initWithDataSource:dataSource initialPhoto:initialPhoto delegate:delegate];
}

- (instancetype)initWithDataSource:(id <CKPhotoBrowserDataSource>)dataSource initialPhoto:(id <CKPhotoBrowserItem> _Nullable)initialPhoto delegate:(nullable id <CKPhotoBrowserControllerDelegate>)delegate {
...
- (void)commonInitWithDataSource:(id <CKPhotoBrowserDataSource>)dataSource initialPhoto:(id <CKPhotoBrowserItem> _Nullable)initialPhoto delegate:(nullable id <CKPhotoBrowserControllerDelegate>)delegate {
    ...
    _transitionController = [[CKPhotoBrowserTransitionController alloc] init];
    self.modalPresentationStyle = UIModalPresentationCustom;
    self.transitioningDelegate = _transitionController;
    self.modalPresentationCapturesStatusBarAppearance = YES;

    _overlayView = ({
        CKPBControllerOverlayView *v = [[CKPBControllerOverlayView alloc] initWithFrame:CGRectZero];
        v.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage ckpb_imageWithName:@"ckpb_close"] landscapeImagePhone:[UIImage ckpb_imageWithName:@"ckpb_close_landscape"] style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonTapped:)];
        v.leftBarButtonItem.imageInsets = CKPhotoBrowserControllerCloseButtonImageInsets;
        v;
    });
	
    _notificationCenter = [NSNotificationCenter new];
	
    self.pageViewController = [[UIPageViewController alloc] initWithTransitionStyle:UIPageViewControllerTransitionStyleScroll navigationOrientation:UIPageViewControllerNavigationOrientationHorizontal options:@{UIPageViewControllerOptionInterPageSpacingKey: @(CKPhotoBrowserControllerInterPhotoSpacing)}];
...
- (void)configurePageViewControllerWithInitialPhoto {
    CKPhotoBrowserChildController *initialPhotoViewController;
...
- (void)updateOverlayInformation {
	...
	UIBarButtonItem *rightItem = self.rightBarButtonItem ?: [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAction target:self action:@selector(actionButtonTapped:)];
	self.overlayView.rightBarButtonItem = self.currentlyDisplayedPhoto.type == CKWebPhotoTypeDefault ? rightItem : nil;
	
	// caption显示逻辑
	if (self.currentlyDisplayedPhoto.type == CKWebPhotoTypeMore) {
		self.overlayView.captionView = nil;
	} else {
		self.overlayView.captionView = [[CKPBCaptionView alloc] initWithAttributedTitle:self.currentlyDisplayedPhoto.attributedCaptionTitle
																	  attributedSummary:self.currentlyDisplayedPhoto.attributedCaptionSummary
																	   attributedCredit:self.currentlyDisplayedPhoto.attributedCaptionCredit];
	}
	...
	// 覆盖物在遇到广告、更多时自动恢复显示
	if (_overlayView.hidden && self.currentlyDisplayedPhoto.type != CKWebPhotoTypeDefault) {
		[self setOverlayViewHidden:NO animated:YES];
	}
}
...
- (void)displayPhoto:(id <CKPhotoBrowserItem>)photo animated:(BOOL)animated {
    if ([self.dataSource indexOfPhoto:photo] == NSNotFound) {
        return;
    }
	
    CKPhotoBrowserChildController *photoViewController = [self newPhotoViewControllerForPhoto:photo];
...
- (void)updatePhotoAtIndex:(NSInteger)photoIndex {
    id<CKPhotoBrowserItem> photo = [self.dataSource photoAtIndex:photoIndex];
    if (!photo) {
        return;
    }
...
- (void)updatePhoto:(id<CKPhotoBrowserItem>)photo {
    ...
    [self.notificationCenter postNotificationName:CKPhotoBrowserChildControllerPhotoImageUpdatedNotification object:photo];
...
- (void)reloadPhotosAnimated:(BOOL)animated {
    id<CKPhotoBrowserItem> newCurrentPhoto;
...
- (void)dismissViewControllerAnimated:(BOOL)animated userInitiated:(BOOL)isUserInitiated completion:(void (^)(void))completion {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:CKPhotoBrowserControllerNotification.willDismiss object:self];
    ...
    [super dismissViewControllerAnimated:animated completion:^{
        ...
        if (!isStillOnscreen) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:CKPhotoBrowserControllerNotification.didDismiss object:self];
        }
...
- (void)setCurrentlyDisplayedViewController:(UIViewController <CKPhotoBrowserContainer> *)viewController animated:(BOOL)animated {
    if (!viewController) {
        return;
    }
...
- (void)setOverlayViewHidden:(BOOL)hidden animated:(BOOL)animated {
    ...
    if (animated) {
        ...
        [UIView animateWithDuration:CKPhotoBrowserControllerOverlayAnimationDuration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowAnimatedContent | UIViewAnimationOptionAllowUserInteraction animations:^{
...
- (CKPhotoBrowserChildController *)newPhotoViewControllerForPhoto:(id <CKPhotoBrowserItem>)photo {
    if (photo) {
        ...
        CKPhotoBrowserChildController *photoViewController = [[CKPhotoBrowserChildController alloc] initWithPhoto:photo loadingView:loadingView notificationCenter:self.notificationCenter];
...
- (void)didNavigateToPhoto:(id <CKPhotoBrowserItem>)photo {
    if ([self.delegate respondsToSelector:@selector(photosViewController:didNavigateToPhoto:atIndex:)]) {
        [self.delegate photosViewController:self didNavigateToPhoto:photo atIndex:[self.dataSource indexOfPhoto:photo]];
    }
    
    [[NSNotificationCenter defaultCenter] postNotificationName:CKPhotoBrowserControllerNotification.didNavigateToPhoto object:self];
}

- (id <CKPhotoBrowserItem>)currentlyDisplayedPhoto {
    return self.currentPhotoViewController.photo;
}

- (CKPhotoBrowserChildController *)currentPhotoViewController {
...
#pragma mark - CKPhotoBrowserChildControllerDelegate
...
{
	if ([_delegate respondsToSelector:@selector(moreItemView:didClickOnPhoto:)]) {
		[_delegate moreItemView:view didClickOnPhoto:photo];
	}
}

#pragma mark - UIPageViewControllerDataSource

- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerBeforeViewController:(UIViewController <CKPhotoBrowserContainer> *)viewController {
...
- (UIViewController *)pageViewController:(UIPageViewController *)pageViewController viewControllerAfterViewController:(UIViewController <CKPhotoBrowserContainer> *)viewController {
    NSUInteger photoIndex = [self.dataSource indexOfPhoto:viewController.photo];
...
- (void)pageViewController:(UIPageViewController *)pageViewController didFinishAnimating:(BOOL)finished previousViewControllers:(NSArray *)previousViewControllers transitionCompleted:(BOOL)completed {
    if (completed) {
		...
		UIViewController <CKPhotoBrowserContainer> *photoViewController = pageViewController.viewControllers.firstObject;
